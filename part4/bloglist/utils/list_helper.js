...
const favoriteBlog = (blogs) => {
  ...
  return blogs.filter(blog => blog.likes === mostLikes).map(blog => ({title: blog.title, author: blog.author, likes: blog.likes}))
}
...
const mostBlogs = (blogs) => {
  ...
  const authorsBlogs = authors.map(author => ({ author: author, blogs: blogs.filter(blog => blog.author === author).length }))
  const mostBlogsAuthor = Math.max(...authorsBlogs.map(author => author.blogs))
